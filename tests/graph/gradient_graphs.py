...
def convert_files_to_lists(file_location):
    ...
    return wenzel_times, enoki_times, pytorch_times, us_times, functions, num_params_list
...
def generate_two_graph(avg_us, avg_them, denom, function, label, num_vars):
    ...
    plt.savefig('results/graph_{}_{}.png'.format(label, num_vars))
    plt.clf()
...
def generate_full_graph(avg_us, avg_pytorch, avg_wenzel, avg_enoki, denom, function, label, num_vars):
    ...
    plt.tight_layout()
    plt.figtext(0.99, 0.01, "y={}".format(function), horizontalalignment='right', fontsize=5)
    plt.savefig('results/graph_{}_full.png'.format(num_vars))
    plt.clf()

wenzel_times, enoki_times, pytorch_times, us_times, functions, num_params = convert_files_to_lists("./results/full_results_random.json")
