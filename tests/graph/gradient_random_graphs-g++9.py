    ...
    with open(file_location) as json_data:
        ...
        for i, key in enumerate(sorted(data)):
            ...
            for num_params in num_params_list:
                ...
                wenzel_static_times[key].append(data[key][num_params_str]['wenzel_static'])
                wenzel_dynamic_times[key].append(data[key][num_params_str]['wenzel_dynamic'])
                enoki_times[key].append(data[key][num_params_str]['enoki'])
                us_times[key].append(data[key][num_params_str]['us'])
                pytorch_times[key].append(data[key][num_params_str]['pytorch'])
                tapenade_times[key].append(data[key][num_params_str]['tapenade'])
...
def generate_full_graph_without_dynamic(avg_us, avg_pytorch, avg_wenzel_static, avg_enoki, avg_tapenade, denom, function, label, num_vars):
    ...
    plt.legend( ('Ours', 'Pytorch', 'Mitsuba (Static)', 'Enoki', 'Tapenade'),
            shadow=False, fontsize=10, frameon=False)
    plt.margins(0,0)
    plt.savefig('./tests/results/grad/graphs/g++9/random/graph_{}_full_no_dynamic.pdf'.format(num_vars), bbox_inches = 'tight',
        pad_inches = 0)
    plt.clf()
...
def generate_max_graph(avg_us, avg_pytorch, avg_wenzel_static, avg_wenzel_dynamic, avg_enoki, avg_tapenade, denom):
    ...
    plt.legend( ('Ours', 'Pytorch', 'Mitsuba (Static)', 'Enoki', 'Tapenade'),
            shadow=False, fontsize=10, frameon=False)
    plt.margins(0,0)
    plt.savefig('./tests/results/grad/graphs/g++9/random/graph_time_to_variables.pdf', bbox_inches = 'tight',
        pad_inches = 0)
    plt.clf()

wenzel_static_times, wenzel_dynamic_times, enoki_times, pytorch_times, us_times, tapenade_times, functions, num_params, wenzel_static_max, wenzel_dynamic_max, enoki_max, pytorch_max, us_max, tapenade_max = convert_files_to_lists("./tests/results/grad/json/random/full_results_random-2020-04-21-20:37:58.json")
