...
def generate_full_graph(avg_us, avg_pytorch, avg_wenzel_static, avg_enoki, avg_tapenade, denom, function, label, num_vars):
    ...
    plt.legend(('ACORNS', 'Pytorch', 'Mitsuba', 'Enoki', 'Tapenade'),
               shadow=False, fontsize=fontsize, frameon=False)
    plt.margins(0, 0)
    plt.savefig('./tests/results/grad/graphs/g++9/random/graph_{}_full_grad_g++9.pdf'.format(num_vars), bbox_inches='tight',
                pad_inches=0)
    plt.clf()

def generate_max_graph(avg_us, avg_pytorch, avg_wenzel_static, avg_enoki, avg_tapenade, denom):
    fig = plt.figure(figsize=(20, 10))
    ax = fig.add_subplot(1, 1, 1)
    ax.plot(denom, avg_us, color='#130f40', markersize=15)
    ax.plot(denom, avg_pytorch, color='#ff7979',
            linestyle='dashed', markersize=7)
    ax.plot(denom, avg_wenzel_static, color='#badc58',
            linestyle='dashed', markersize=7)
    ...
    plt.legend(('ACORNS', 'Pytorch', 'Mitsuba', 'Enoki', 'Tapenade'),
               shadow=False, fontsize=fontsize, frameon=False)
    plt.margins(0, 0)
    plt.savefig('./tests/results/grad/graphs/g++9/random/graph_max_grad_g++9.pdf', bbox_inches='tight',
                pad_inches=0)
    plt.clf()
...
wenzel_static_times, wenzel_dynamic_times, enoki_times, pytorch_times, us_times, tapenade_times, functions, num_params, wenzel_static_max, wenzel_dynamic_max, enoki_max, pytorch_max, us_max, tapenade_max = convert_files_to_lists(
    "./tests/results/grad/json/random/full_results_random-2020-04-21-20:37:58.json")
...
generate_max_graph(us_max, pytorch_max, wenzel_static_max,
                   enoki_max, tapenade_max, range(1, 11))
